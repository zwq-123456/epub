<template>
  <div class="ebook-slide-contents">
    ...
    <scroll class="slide-contents-list" :top="156" :bottom="48" v-show="!searchVisible">
      <div class="slide-contents-item" v-for="(item, index) in navigation" :key="index">
        <span
          ...
          :style="{ selected: section === index }"
          :class="classItemNva(item)"
    ...
    <!--     
      <div class="slide-contents-list" v-show="!searchVisible" ref="wrapper">
        <div class="slide-contents-item" v-for="(item, index) in navigation" :key="index">
          <span
            class="slide-contents-item-label"
            :class="{ selected: section === index }"
            :style="classItemNva(item)"
            @click="displayNav(item.href)"
            >{{ item.label }}</span
          >
          <span class="slide-contents-item-page"></span>
...
export default {
  ...
  data() {
    return {
      searchVisible: false,
      searchList: null, //搜索得结果
      searchText: "", //搜索得文本
    };
  },
  methods: {
    //全文检索后点击跳转渲染并且高亮
    displaySearch(item, highLight = false) {
      this.display(item, () => {
      ...
      if (this.searchText && this.searchText.length > 0) {
        this.doSearch(this.searchText).then((res) => {
          ...
          this.searchList.map((item) => {
            return (item.excerpt = item.excerpt.replace(
              this.searchText,
